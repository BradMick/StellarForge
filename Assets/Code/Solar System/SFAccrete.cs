...
public class SFAccrete
{
    ...
    private void CollectDust(SFNuclei _protoPlanet, float _stellarLuminosityRatio)
    {
        ...
        //Swept volume can come out negative when the effect limits invert on a narrow
        //band; mass must never go below zero or the whole body becomes non-physical
        _protoPlanet.Mass       = Mathf.Max(volume * massDensity, 0.0f);
        _protoPlanet.GasMass    = Mathf.Max(volume * gasDensity, 0.0f);
        _protoPlanet.DustMass   = Mathf.Max(_protoPlanet.Mass - _protoPlanet.GasMass, 0.0f);
    }
    ...
    {
        return DistributePlanetaryMasses(_stellarMassRatio, _stellarLuminosityRatio, _outerPlanetBoundary, 0.0f);
    }

    //_innerPlanetBoundaryOverride > 0 carves out a circumbinary exclusion zone: in a
    //P-type system nothing survives close to the pair, so accretion starts further out
    public List<SFNuclei> DistributePlanetaryMasses(float _stellarMassRatio, float _stellarLuminosityRatio,
                                                    float _outerPlanetBoundary, float _innerPlanetBoundaryOverride)
    {
        ...
        while (DUST_LEFT)
        {
            ...
            float innerPlanetBoundary = _innerPlanetBoundaryOverride > 0.0f
                                      ? _innerPlanetBoundaryOverride
                                      : SFUtilities.CalculateInnerPlanetBoundary(_stellarMassRatio),
                  outerPlanetBoundary;
            ...
            //A truncated disc can leave no room at all — bail rather than sampling an
            //inverted range (which injects protoplanets inside the inner limit, where
            //there is no dust, and starves the whole run)
            if (outerPlanetBoundary <= innerPlanetBoundary)
                break;

            protoPlanet.Axis        = SFUtilities.Range(innerPlanetBoundary, outerPlanetBoundary);
