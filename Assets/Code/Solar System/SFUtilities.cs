...
{
    //The generator's own RNG: system generation is a pure function of the seed, immune
    //to anything else in the game consuming UnityEngine.Random (whose global .seed API
    //is deprecated anyway)
    private static System.Random rng = new System.Random(0);

    public static void InitRandom(int _seed)
    {
    ...
    public static float CalculateEccentricity()
    {
        ...
        e = 1.0f - Mathf.Pow(Range(0.0f, 1.0f), SFConstants.ECCENTRICITY_COEFFICIENT);
    ...
    //5.c — Circumbinary (P-type) critical radius: planets orbiting BOTH stars are unstable
    //inside this distance from the barycenter, because the pair's shifting gravity keeps
    //stirring the region. Holman & Wiegert (1999) empirical fit; typically 2-4x the
    //separation, growing with eccentricity and toward equal masses
    public static float CalculateInnerPlanetLimitCircumbinary(float _mass1, float _mass2, float _eccen, float _separation)
    {
        float mu = _mass2 / (_mass1 + _mass2);
        float e = _eccen;
    ...
    {
        return _value + (_value * Range(-_variation, _variation));
    }
}
